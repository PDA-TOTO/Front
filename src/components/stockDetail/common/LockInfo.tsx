...
export default function LockInfo({ tabName, imgLink }: Props) {
    ...
    const { id } = useParams();
    return (
        <Flex direction={'column'} justify="center" align="center" className={classes.lock_box}>
            <Image src={imgLink} className={classes.lock_img_blur} />
            <Flex direction={'column'} justify={'center'} align={'center'} className={classes.lock_box_2}>
                <Flex direction={'row'} justify="center" align="center" className={classes.lock_title}>
                    <div className={classes.lock_img}>
                        <Image src={lock} />
                    </div>
                    아직 {tabName} 탭을 이용하실 수 없어요🥲{' '}
                </Flex>
                <Flex direction={'column'} align="center" className={classes.lock_content}>
                    <div>투투는 여러분들이 해당 서비스가 왜 필요한지 느꼈으면 좋겠어요</div>
