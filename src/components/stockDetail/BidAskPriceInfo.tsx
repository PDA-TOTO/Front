...
const BidAskPriceInfo: React.FC<BidAskPriceInfoProps> = ({ onPriceClick, tradingType }: BidAskPriceInfoProps) => {
    const stockWebSocket = useStockDetailSelector((state) => state.stockWebSocket);
    useEffect(() => {
            ...
            >
                ...
                <Table.Tbody>
                    {Array.from({ length: 20 }, (_, index) => (
                        ...
                        >
                            <Table.Td c="blue.6">
                                {index < 10 && stockWebSocket[`askp_rsqn${10 - index}`].toLocaleString()}
                            </Table.Td>
                            <Table.Td>
                                {index < 10
                            ...
                            <Table.Td c="red.6">
                                {index >= 10 && stockWebSocket[`bidp_rsqn${index - 9}`].toLocaleString()}
                            </Table.Td>
                        </Table.Tr>
                    ))}
                </Table.Tbody>
