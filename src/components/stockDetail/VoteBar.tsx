...
export default function VoteBar({ leftAmount, rightAmount }: Props) {
    ...
    let leftPercent = totalAmount === 0 ? 0.5 : leftAmount / totalAmount;
    let rightPercent = totalAmount === 0 ? 0.5 : rightAmount / totalAmount;

    if (leftPercent === 1) rightPercent = 0;
    else if (rightPercent === 1) leftPercent = 0;
    ...
    return (
        <Flex direction={'column'} justify="center">
            <Flex direction={'row'} align="center" justify="center" style={{ position: 'relative' }}>
                <Flex
                    className={classes.vote_left}
                    align="center"
                    style={{ width: `${leftPercent * 50}vw`, paddingLeft: '10px' }}
                >
                    <Image src={ThumbsUp} w={40} h={40} style={{ marginRight: '5px' }} className={classes.image_box} />
                ...
                >
                    ...
                    <Image src={ThumbsDown} w={40} h={40} style={{ marginLeft: '5px' }} className={classes.image_box} />
                </Flex>
            </Flex>
        </Flex>
    );
