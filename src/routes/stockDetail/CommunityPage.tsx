...
import { getCommentByCommunityId, saveComment, likeComment } from '../../lib/apis/comment';
...
export default function CommunityPage({}) {
    ...
    const handleVoteChange = (vote: string) => {
        ...
        if (userVote === vote) {
            ...
            if (vote === 'LIKE') {
                ...
                id &&
                    communityInfo &&
                    voteChange(id, communityInfo.result.id, 'LIKE').then((data) => console.log('vote:', data));
                if (userVote === 'UNLIKE') {
                ...
                id &&
                    communityInfo &&
                    voteChange(id, communityInfo?.result.id, 'UNLIKE').then((data) => console.log('vote:', data));
                if (userVote === 'LIKE') {
                    setLikeCnt((prev) => prev - 1);
                }
            }
        ...
        notifications.show({
            ...
            autoClose: 3000,
            radius: 'md',
            color: userVote === vote ? 'primary.5' : vote === 'LIKE' ? 'red.5' : 'blue.5',
        });
    };
    ...
    const handleCommentSubmit = () => {
        ...
        communityInfo &&
            saveComment(communityInfo.result.id, commentText.trim()).then((response) => {
    ...
    const handleLikeClick = (index: number, isLiked: string) => {
        setCommentLikedList((prevList) =>
            prevList.map((item) =>
                item.id === index
                    ? {
                          ...item,
                          likeAmount: isLiked === 'LIKE' ? item.likeAmount - 1 : item.likeAmount + 1,
        ...
        likeComment(index, isLiked === 'UNLIKE' ? 'LIKE' : 'UNLIKE').then((response) => {
            console.log(response.data);
        });
    };

    const calculateCurrentPrice = (closePrice: string) => {
        ...
        id &&
            communityBykrxCode(id).then((response) => {
                ...
                getCommentByCommunityId(response.data.result.id).then((response) => {
                    const sortedCommentList2 = [...response.data].sort((a: commentType, b: commentType) => a.id - b.id);
    ...
    return (
        <Flex direction={'column'} justify="center" align="center" className={classes.community_flex}>
            <Flex direction={'column'} justify="center" align="center" className={classes.header}>
                <div className={classes.header_title}>{communityInfo?.result.stockCodeName}</div>
                <Flex direction={'row'}>
                    <div className={classes.header_price}>
                        {naverStockInfo && naverStockInfo.marketStatus === 'CLOSE'
                            ? naverStockInfo.closePrice
                            : naverStockInfo && calculateCurrentPrice(naverStockInfo.closePrice)}
                        원
                    </div>
                    <div
                        className={
            ...
            <Flex className={classes.main} direction={'column'} align="center">
                <Flex direction={'column'} justify="center" align="center" className={classes.vote_header}>
                    <div className={classes.main_header}>투표</div>
                    <div className={classes.main_title}>{communityInfo?.result.voteTitle}</div>
                    <Flex direction={'row'} justify="center" className={classes.main_headcount}>
                        <Image src={headCount} h={20} w="auto" style={{ paddingRight: '5px' }} />
                        {communityInfo && <div>{communityInfo.result.numOfParticipants}명 참여 중</div>}
                    </Flex>
                    <Flex direction={'row'}>
                        <Image src={date} h={20} w="auto" style={{ paddingRight: '5px' }} />
                        <div className={classes.main_date}>
                            {communityInfo?.result.startDate} ~ {communityInfo?.result.endDate}
                        </div>
                    </Flex>
                    <div className={classes.main_padding} />
                    {communityInfo && <VoteBar leftAmount={likeCnt} rightAmount={unlikeCnt} />}
                ...
                >
                    <div className={classes.main_btn_write} onClick={() => setWriteToggle((prev) => !prev)}>
                        작성하기
                    </div>
                ...
                {writeToggle &&
                    (userVote === 'NONE' ? (
                        <Flex direction={'column'} justify="center" className={classes.write_NONE_flex}>
                            <div className={classes.write_NONE_1}>아직 투표가 완료되지 않았어요!</div>
                            <div className={classes.write_NONE_2}>위에서 투표를 끝내고 이유를 작성해주세요 😀</div>
                        </Flex>
                    ) : (
                        <Flex direction={'column'} justify="center" align="center">
                            <Flex direction={'column'} align="flex-start">
                                {userVote === 'LIKE' && (
                                    <Flex className={classes.write_1} align="center">
                                        <div className={classes.write_badge_red}>찬성</div>을 선택한 이유를 써주세요!
                                    </Flex>
                                )}
                                {userVote === 'UNLIKE' && (
                                    <Flex align="center" className={classes.write_1}>
                                        <div className={classes.write_badge_blue}>반대</div>를 선택한 이유를 써주세요!
                                    </Flex>
                                )}
                                <textarea
                                    value={commentText}
                            ...
                            <button className={classes.comment_btn} onClick={handleCommentSubmit}>
                                작성 완료
                            </button>
                        </Flex>
                    ))}
                ...
                {value === '최신순'
                    ...
                    : commentLikedList?.slice().map((value) => {
                          return (
                              <Flex key={value.id} direction={'column'} justify="center" align="center">
                                  <Comment
                                      id={value.id}
