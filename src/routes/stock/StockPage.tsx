            ...
            <Flex dir="row" justify="space-between" gap="sm">
                {stockMajorList.length > 0 ? (
                    stockMajorList?.map((item, idx: number) => {
                        return (
                            <div key={idx} className={idx % 2 === 0 ? classes.card_v1 : classes.card_v2}>
                                <div className={classes.card_title}>{item.stockName}</div>
                                <div className={classes.card_text}>
                                    {item.closePrice}
                                    {Number(item.fluctuationsRatio) > 0 ? (
                                        <div className={classes.card_percent_plus}>+{item.fluctuationsRatio}%</div>
                                    ) : (
                                        <div className={classes.card_percent_minus}>{item.fluctuationsRatio}%</div>
                                    )}
                                </div>
                            </div>
                        );
            ...
            <Flex dir="row" justify="space-between">
                <Flex dir="row" align="center">
                    {buttonGroup.map((item, idx) => {
                        return (
                            <button
                                className={activateId === item.id ? classes.btn_activate_mid : classes.btn_mid}
                                key={idx}
                ...
                <Flex direction={'column'}>
                    ...
                    {searchValue !== '' && (
                        <Flex
                            className={`${classes.input_result} ${searchValue !== '' ? classes.show : ''}`}
                            direction={'column'}
                        >
                            {searchList?.map((value) => {
                                return (
                                    ...
                                    >
                                        {value.name}
                                        <div className={classes.search_krxCode}>{value.krxCode}</div>
                                    </Flex>
                                );
                            })}
                        </Flex>
