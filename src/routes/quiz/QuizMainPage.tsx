...
import test from "../../assets/img/main/1.Newspaper.svg";
...
export default function QuizMainPage() {
  ...
  const question =
    location.state.solve &&
    quizs.map((value, index) => {
      return (
        <Flex
          key={index}
          direction="column"
          ...
          <Flex direction="column" w="400px" gap="15px" mb="20px">
            <Flex align="center" gap="10px">
              ...
              {value.answer === location.state.choiceList[index] + 1 ? (
                <Badge color="blue.3" c="primary.5" size="lg" fw="600">
                  정답
                </Badge>
              ) : (
                <Badge color="red.3" c="primary.5" size="lg" fw="600">
                  오답
                </Badge>
              )}
            </Flex>
            <Text size="18px" lh="20px" fw="600">
              {value.question}
            </Text>
          </Flex>
          <Flex direction="column" gap="10px">
            {[value.option1, value.option2, value.option3, value.option4].map(
              (v, idx) => {
          ...
          <Flex direction="column" justify="center" align="center">
            <Flex direction="column" w="100%">
              <Flex w="100%" justify="space-between">
                <Text size="24px" fw="600" c={`${color[numLevel]}`}>
                  {level[numLevel]}
                </Text>
                <Text size="24px" fw="600" c={`${color[numLevel + 1]}`}>
                  {level[numLevel + 1]}
                </Text>
              </Flex>
              <Progress
              ...
              <Flex className={classes.cardFrame}>
                <div className={classes.card}>
                  <Flex
                    ...
                    justify={"center"}
                    align={"center"}
                    bg={`linear-gradient(${color[numLevel]},var(--mantine-color-white-5),${color[numLevel]})`}
                  >
                ...
                <Flex pos="absolute" right="400px" bottom="-100px">
                  <div className={classes.card}>
                    <Flex
                      ...
                      bg={`linear-gradient(${
                        color[numLevel - 1]
                      },var(--mantine-color-white-5),${color[numLevel - 1]})`}
                      opacity="30%"
                    >
                      <Text size="60px" fw="400" className={classes.level}>
                        {level[numLevel - 1]}
        ...
        >
          ...
          {user.user.experience === 0 ? (
            <Flex
              ...
              justify={"center"}
              align={"center"}
              bg="linear-gradient(var(--mantine-color-primary-5),var(--mantine-color-white-5),var(--mantine-color-primary-5))"
            >
              <Text size="60px" fw="400" className={classes.level}>
                ?
              </Text>
          ...
          <Button
            ...
            size="20px"
            onClick={() => {
              navigate("/quiztest", { state: { level: true } });
              if (user.user.experience === 0) {
                updateExperience(5).then(() => {
